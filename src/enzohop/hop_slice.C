...
{
    if (fwrite(&len,SIZE_OF_INT,1,f)!=1) return 1;
...
{
    int size, size2;
    if (fread(&size,SIZE_OF_INT,1,f)!=1)
    ...
    if (fread(&size2,SIZE_OF_INT,1,f)!=1)
...
tag file header. */
{
	...
	if (conp) {  /* Store the data */
	    f77read(f, s->px+s->numperblock*block+1, s->numperblock*SIZE_OF_FLOAT);
	    f77read(f, s->py+s->numperblock*block+1, s->numperblock*SIZE_OF_FLOAT);
	    f77read(f, s->pz+s->numperblock*block+1, s->numperblock*SIZE_OF_FLOAT);
	} else {     /* Don't store the data */
	    f77read(f, dummylist+1, s->numperblock*SIZE_OF_FLOAT);
	    f77read(f, dummylist+1, s->numperblock*SIZE_OF_FLOAT);
	    f77read(f, dummylist+1, s->numperblock*SIZE_OF_FLOAT);
	}
	/* Now read the three velocity blocks */
	if (conv) {  /* Store the data */
	    f77read(f, s->vx+s->numperblock*block+1, s->numperblock*SIZE_OF_FLOAT);
	    f77read(f, s->vy+s->numperblock*block+1, s->numperblock*SIZE_OF_FLOAT);
	    f77read(f, s->vz+s->numperblock*block+1, s->numperblock*SIZE_OF_FLOAT);
	} else {     /* Don't store the data */
	    f77read(f, dummylist+1, s->numperblock*SIZE_OF_FLOAT);
	    f77read(f, dummylist+1, s->numperblock*SIZE_OF_FLOAT);
	    f77read(f, dummylist+1, s->numperblock*SIZE_OF_FLOAT);
...
/* If ftag==NULL, don't read the tag file.  Otherwise do read it. */
{
    ...
    f77read(f,s->px+1,SIZE_OF_FLOAT*s->numlist);
    f77read(f,s->py+1,SIZE_OF_FLOAT*s->numlist);
    f77read(f,s->pz+1,SIZE_OF_FLOAT*s->numlist);
    f77read(f,s->vx+1,SIZE_OF_FLOAT*s->numlist);
    f77read(f,s->vy+1,SIZE_OF_FLOAT*s->numlist);
    f77read(f,s->vz+1,SIZE_OF_FLOAT*s->numlist);
...
{
    if (fread(ntag+1, SIZE_OF_INT, numread, f)!=numread)
...
{
    int dummy[2];
    if (fread(&dummy, SIZE_OF_INT, 2, f)!=2) myerror("Error in reading tag file.");
...
/* Return the number of groups */
{
    ...
    if (fread(&dummy, SIZE_OF_INT, 2, f)!=2) myerror("Error 1 in reading tag file.");
    if (dummy[0]!=s->numpart)
	myerror("First int of tag file doesn't match numpart.");
    s->numgroups = dummy[1];
 
    if (fread(s->ntag+1, SIZE_OF_INT, s->numlist, f)!=s->numlist)
