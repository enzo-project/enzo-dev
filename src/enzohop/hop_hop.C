...
particles in the group */
{
    ...
    /* printf("size of float %d size of int %d\n",SIZE_OF_FLOAT,SIZE_OF_INT); */
    smx->densestingroup = (int *)malloc((size_t)((smx->nGroups+1)*SIZE_OF_INT));
    assert(smx->densestingroup!=NULL);
    smx->nmembers = (int *)malloc((size_t)(SIZE_OF_INT*(smx->nGroups+1)));
...
/* Store the number of members and most dense particle in each group */
{
    ...
    indx = (int *)malloc((size_t)(SIZE_OF_INT*(smx->nGroups+1)));
    assert(indx!=NULL);
    irank = (int *)malloc((size_t)(SIZE_OF_INT*(smx->nGroups+1)));
    assert(irank!=NULL);
...
/* Binary file: nActive, nGroups, list of Groups */
{
    ...
    dummy=smx->kd->nActive; fwrite(&dummy, SIZE_OF_INT, 1, fp);
    dummy=smx->nGroups; fwrite(&dummy, SIZE_OF_INT, 1, fp);
    for (j=0;j<smx->kd->nActive;j++) {
      fwrite(&(smx->kd->p[j].iHop),SIZE_OF_INT,1,fp);
      /* fprintf(blahfp,"%d %d\n",smx->kd->p[j].iID,smx->kd->p[j].iHop); */ /* S Skory */
    }
    ...
    for (j=0;j<smx->kd->nActive;j++) { /* S Skory */
      /* fprintf(stderr,"iID = %d\n",smx->kd->p[j].iID); */
      fwrite(&(smx->kd->p[j].iID),SIZE_OF_INT,1,fp);
    }
...
/* Binary file: nActive, list of Densities */
{
    ...
    dummy=smx->kd->nActive; fwrite(&dummy, SIZE_OF_INT, 1, fp);
    for (j=0;j<smx->kd->nActive;j++) fwrite(&(smx->kd->p[j].fDensity),SIZE_OF_FLOAT,1,fp);
    return;
}
...
/* Binary file: nActive, list of Densities */
{
    ...
    if (fread(&dummy,SIZE_OF_INT,1,fp)!=1) {
	fprintf(stderr,"Format of density file seems wrong.\n"); exit(1);
    }
    assert(dummy==smx->kd->nActive);
    for (j=0;j<smx->kd->nActive;j++)
	if (fread(&(smx->kd->p[j].fDensity),SIZE_OF_FLOAT,1,fp)!=1) {
...
/* Groups should be ordered before calling this (else densities will be wrong)*/
{
    ...
    fprintf(fp,"%"ISYM"\n", smx->nGroups);
    fprintf(fp,"# Number of Particles: %"ISYM"\n", smx->kd->nActive);
    fprintf(fp,"# Number of Groups: %"ISYM"\n", smx->nGroups);
    fprintf(fp,"# Number of Particles not in a Group: %"ISYM"\n",
	    smx->nmembers[smx->nGroups]);
    fprintf(fp,"# nSmooth = %"ISYM", nHop = %"ISYM", nMerge = %"ISYM", fDensThresh = %6.3"FSYM".\n",
	...
	fprintf(fp,"%4"ISYM" %6"ISYM" %8"ISYM" %9.7"FSYM" %9.7"FSYM" %9.7"FSYM" %8.2"FSYM"\n",
	...
	if (hp->nGroup1>=0)
	    fprintf(fp,"%5"ISYM" %5"ISYM" %6.2"FSYM"\n",
