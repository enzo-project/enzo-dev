...
/* Groups need not be sorted, but must be numbered from 0 to ngroups-1 */
{
    ...
    if (fread(&(g->npart),SIZE_OF_INT,1,f)!=1) myerror("Tag file read error.");
    if (fread(&(g->ngroups),SIZE_OF_INT,1,f)!=1) myerror("Tag file read error.");
    ...
    fread(s->ntag+1, SIZE_OF_INT, s->numlist, f); /* Read in all the tags */
    fread(s->ID+1, SIZE_OF_INT, s->numlist,f); /* Read in the real particle IDs. S Skory */
    fclose(f);
...
/* We read the file in segments, so as to reduce memory consumption */
{
    ...
    npart = 0; fread(&npart,SIZE_OF_INT,1,f);
    if (npart!=s->numpart)
	...
	if (fread(density,SIZE_OF_FLOAT,block,f)!=block)
...
/* I think this will work even if saddledensthresh<densthresh */
{
	...
	if (sscanf(line, "%"ISYM" %"ISYM" %"ISYM" %"FSYM" %"FSYM" %"FSYM" %"FSYM, dummy, dummy+1, dummy+2,
	...
	if (sscanf(line,"%"ISYM" %"ISYM" %"FSYM, &g1, &g2, &dens)!=3 || g1<0 || g2<0 ||
...
/* If fname==NULL, write to stdout */
{
    ...
    fwrite(&(s->numpart),SIZE_OF_INT,1,f);
    printf("writetags: s->numpart = %d gl->ngroups = %d\n",
    	s->numpart, gl->ngroups);
    fwrite(&(gl->ngroups),SIZE_OF_INT,1,f);
    fwrite(s->ntag+1,SIZE_OF_INT,s->numlist,f);
    fwrite(s->ID+1,SIZE_OF_INT,s->numlist,f); /* S Skory */
    fclose(f);
...
/* Use a format readable for FORTRAN unformatted read commands */
{
    ...
    dummy = 8; fwrite(&dummy,SIZE_OF_INT,1,f);
    fwrite(&(s->numpart),SIZE_OF_INT,1,f);
    fwrite(&(gl->ngroups),SIZE_OF_INT,1,f);
    fwrite(&dummy,SIZE_OF_INT,1,f);
    dummy = s->numlist*SIZE_OF_INT; fwrite(&dummy,SIZE_OF_INT,1,f);
    fwrite(s->ntag+1,SIZE_OF_INT,s->numlist,f);
    fwrite(&dummy,SIZE_OF_INT,1,f);
    fclose(f);
    return;
