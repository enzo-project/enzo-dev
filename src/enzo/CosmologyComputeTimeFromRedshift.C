...
#define INVERSE_HYPERBOLIC_EXISTS
  ...
  FLOAT TimeHubbleNew;
    if (CosmologyTableComputeTimeFromRedshift(Redshift, &TimeHubbleNew) == FAIL) {
      ENZO_FAIL("Error in CosmologyTableComputeTime.\n");
    }
    fprintf(cosmo_fptr_1, "%"GSYM" %.12"ESYM" %.12"ESYM"\n",
            Redshift, TimeHubble0 / (HubbleConstantNow*3.24e-18),
            TimeHubbleNew / (HubbleConstantNow*3.24e-18));
