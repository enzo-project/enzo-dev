...
int grid::FlagCellsToBeRefinedByVelDiv()
{
  ...
  for (k=0; k<kn; k++){
  	for (j=0; j<jn; j++){
  		for (i=0; i<in; i++){
  			...
  			div[IDX(i,j,k)] = max(div[IDX(i,j,k)], tiny_number);
  ...
  for (k = GridStartIndex[2]; k <= GridEndIndex[2]; k++){
    for (j = GridStartIndex[1]; j <= GridEndIndex[1]; j++) {
      ...
      for (i = GridStartIndex[0]; i <= GridEndIndex[0]; i++){
              dxcalc = sqrt(hmcoef/a/div[index+i]*d[index+i])/RefineByVelDivSafetyFactor;
	if (dx > dxcalc){
 ...
 // FLOAT CellWidthSquared = CellWidth[0][0]*CellWidth[0][0];
  for (i = 0; i < size; i++)
    {
      dxcalc = sqrt(hmcoef/a/div[i]*d[i])/RefineByVelDivSafetyFactor;
	if (dx > dxcalc){
	  FlaggingField[i]++; 
