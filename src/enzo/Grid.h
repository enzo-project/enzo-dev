...
gradient force to gravitational force for one-zone collapse test. */
  ...
  void SetProcessorLayout(int Dimension, int procs)
  {
    if (Dimension < 0 || Dimension > MAX_DIMENSION)
      ...
      ProcLayout[Dimension] = procs;
    else
      fprintf(stderr, "SetProcessorLayout: invalid procs value.\n");
  }
  ...
  void SetProcessorLocation(int Dimension, int location)
  {
    if (Dimension < 0 || Dimension > MAX_DIMENSION)
      ...
      ProcLocation[Dimension] = location;
    else
      fprintf(stderr, "SetProcessorLocation: invalid location.\n");
  }
  ...
  void SetProcessorNeighbors(int Dimension, int LR, int NBid)
  {
    if (Dimension < 0 || Dimension > MAX_DIMENSION)
      ...
      ProcNeighbors[Dimension][LR] = NBid;
    else
      fprintf(stderr, "SetProcessorNeighbors: invalid grid ID.\n");
  }
...
iveParticles; };
  ...
  /* Gravity Test: check results. APM version */
  int TestGravityAPMCheckResults(FILE *fptr, grid *TopGrid, int level);
...
#define MAX_SPHERES 10
  ...
  {
    return NumberOfBaryonFields;
  };
  int SetNumberOfBaryonFields(int &number)
  {
    NumberOfBaryonFields = number;
    return 0;
  };
  ...
  inline bool isLocal()
  {
    return MyProcessorNumber == ProcessorNumber;
  };
...
#include "../anyl/Grid_AnalyzeClusters.h"
  ...
  Star *ReturnStarPointer(void)
  {
    return Stars;
  };
  int ReturnNumberOfStars(void)
  {
...
#include "PhotonGrid_Methods.h"
  ...
  bool isTopGrid()
  {
    for (int i = 0; i < GridRank; i++)
    {
      if (CellWidth[i][0] < TopGridDx[i] * 0.95)
        return FALSE;
      if (CellWidth[i][0] > TopGridDx[i] * 1.05)
        return FALSE;
    }
    return TRUE;
  };
  ...
  template <typename T>
  int multA(T *input, T *output, int *MatrixStartIndex, int *MatrixEndIndex);
  template <typename T>
  int multA2(T *input, T *output, int *MatrixStartIndex, int *MatrixEndIndex);
  template <typename T>
  T dot(T *a, T *b, int size);
  template <typename T>
  int setNeumannBC(T *x, int *MatrixStartIndex, int *MatrixEndIndex, int type);
  template <typename T>
  int setDirichletBC(T *x, int *MatrixStartIndex, int *MatrixEndIndex);

  int PoissonCleanStep(int level);

  int GetIndex(int i, int j, int k)
  {
    return i + j * (GridDimension[0]) + k * (GridDimension[0]) * (GridDimension[1]);
  }
