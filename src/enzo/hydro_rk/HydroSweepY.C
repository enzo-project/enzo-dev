...
int HydroSweepY_Lagrange(float **Prim, float **Flux3D, int GridDimension[], 
  ...
  */
{
  ...
  if (LagrangeReconstruction != 0) {
    if (HydroSweepY_Lagrange(Prim, Flux3D, GridDimension, GridStartIndex, CellWidth, dtdx, fallback)==FAIL)
      return FAIL;
    return SUCCESS; 
  }
  ...
  */
{
  ...
  for (k = 0; k < Zactivesize; k++) {
	...
	ii = (i + sign(-vx) + GridStartIndex[0]) + j * GridDimension[0] +
	  (k + GridStartIndex[2]) * GridDimension[1] * GridDimension[0];
	kk = (i + GridStartIndex[0]) + j * GridDimension[0] +
	  (k + sign(-vz) + GridStartIndex[2]) * GridDimension[1] * GridDimension[0];

	weighta = 1.*fabs(vx)*dtdx;
	weightb = 1.*fabs(vz)*dtdx;

	Prim1[0][j] = Prim[iden][igrid]; // density
	Prim1[0][j] = LagrangeInterpolate(Prim[iden][igrid],Prim[iden][ii],Prim[iden][kk],
					  weighta, weightb);
	...
	vx = LagrangeInterpolate(Prim[ivy][igrid],Prim[ivy][ii],Prim[ivy][kk],
				 weighta, weightb);
	vy = LagrangeInterpolate(Prim[ivz][igrid],Prim[ivz][ii],Prim[ivz][kk],
				 weighta, weightb);
	vz = LagrangeInterpolate(Prim[ivx][igrid],Prim[ivx][ii],Prim[ivx][kk],
				 weighta, weightb);

	if (DualEnergyFormalism) {
	  Prim1[1][j] = Prim[ieint][igrid];
	  Prim1[1][j] = LagrangeInterpolate(Prim[ieint][igrid],Prim[ieint][ii],Prim[ieint][kk],
					  weighta, weightb);
	}
	else {
	  etot = Prim[ietot][igrid];
	  etot = LagrangeInterpolate(Prim[ietot][igrid],Prim[ietot][ii],Prim[ietot][kk],
					  weighta, weightb);
	  v2 = vx*vx + vy*vy + vz*vz;
	  Prim1[1][j] = etot - 0.5*v2;
	}
	if (EOSType > 0) {
	  float h, cs, dpdrho, dpde;
	  //	  EOS(p, Prim[iden][igrid], Prim1[1][j], h, cs, dpdrho, dpde, EOSType, 0);
	  EOS(p, Prim1[0][j], Prim1[1][j], h, cs, dpdrho, dpde, EOSType, 0);
	  Prim1[1][j] = p;
	}
