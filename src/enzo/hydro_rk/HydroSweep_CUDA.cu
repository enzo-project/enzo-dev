...
__device__ float minmod_cu(const float &a, const float &b, const float &c);
__device__ float Max_cu(const float &a, const float &b, const float &c);
__device__ float Min_cu(const float &a, const float &b, const float &c);
...
__device__ void LLF_PLM_CUDA3(float *Prim, float *Flux, const int &tx)
{
  ...
  lp_l = Max_cu(0, lp_l, lp_r);
  lm_l = Max_cu(0, -lm_l, -lm_r);
...
// minmod_cu limiter function
__device__ float minmod_cu(const float &a, const float &b, const float &c)
{
  return 0.25*(sign(a)+sign(b))*ABS((sign(a)+sign(c)))*Min_cu(ABS(a), ABS(b), ABS(c));
}
...
__device__ void plm_point(const float &vm1, const float &v, const float &vp1, float &vl_plm)
{
  ...
  float dv = minmod_cu(dv_l, dv_r, dv_m);
