...
int grid::ComputeAccelerationFieldAPM(int RefinementFactor)
{
  ...
  //#define NO_COMPUTE_POTENTIAL_FIELD
#define COMPUTE_POTENTIAL_FIELD
#ifdef COMPUTE_POTENTIAL_FIELD

  /* Copy buffer1 in buffer3 */

  float *buffer3 = new float[FFTSize];
  for (i = 0; i < FFTSize; i++)
    buffer3[i] = buffer1[i];

  /* Compute the potential with inverse FFT */

  if (FastFourierTransform(buffer3, GridRank, FFTDimensionReal,
  ...
  for (k = 0; k < FFTDimensionGrav[2]; k++) {
    for (j = 0; j < FFTDimensionGrav[1]; j++) {
      for (i = 0; i < FFTDimensionGrav[0]; i++) {
	PotentialField[INDEX_GRAV(i+FFTStartIndex[0], j+FFTStartIndex[1], k+FFTStartIndex[2])] = buffer3[INDEX_FFT(i,j,k)];
      }
    }
  }
...
#endif /* COMPUTE_POTENTIAL_FIELD */

  /* Compute size of acceleration field. */

  int size = GridDimension[0]*GridDimension[1]*GridDimension[2];

  /* Compute offset between AccelerationField and accelerations in buffer2. */

  int Offset[MAX_DIMENSION] = {0,0,0};
  for (dim = 0; dim < GridRank; dim++) {
    Offset[dim] = nint((CellLeftEdge[dim][0] - GravitatingMassFieldLeftEdge[dim])/GravitatingMassFieldCellSize);
  }

  /* To compute the acceleration, loop over the dimensions.
     For each dimension, compute the acceleration field and copy it into
     the appropriate spot in this object. */

  for (dim = 0; dim < GridRank+ComputePotential; dim++) {

    /* If dim==GridRank then this is the potential. */

    if (dim == GridRank)
      for (i = 0; i < FFTSize; i++)
	buffer2[i] = buffer1[i];

    /* Compute the acceleration for dim in k-space and put the
       the result in buffer2 and divide by cell width. */

    else
      if (GreensFunction.ComputeAcceleration(GridRank, FFTDimensionReal, FFTDimension, dim, buffer1, buffer2, GravitatingMassFieldCellSize*a) == FAIL) {
	fprintf(stderr, "Error in GreensFunction->ComputeAcceleration.\n");
	return FAIL;
      }

    /* Inverse transform the field. */

    if (FastFourierTransform(buffer2, GridRank, FFTDimensionReal,
			     FFTDimension, FFT_INVERSE, REAL_TO_COMPLEX) == FAIL) {
      fprintf(stderr, "Error in FastFourierTransform (inverse).\n");
      return FAIL;
    }

    /* Allocate the accleration field (deleting it first if present). */
	...
	delete AccelerationField[dim];
      ...
      if (AccelerationField[dim] == NULL) {
	perror("error: ");
	fprintf(stderr, "Grid_ComputeAccelerationField: malloc error: %d (out of memory?)\n", size);
	return FAIL;
      }

      /* Copy the acceleration field:
	 all grids: copy active part of buffer2 to AccelerationField. */
      /* FIX FOR GravityResolution != 1 */

      for (k = 0; k < GridDimension[2]; k++) {
	for (j = 0; j < GridDimension[1]; j++) {
	  for (i = 0; i < GridDimension[0]; i++) {
	    AccelerationField[dim][INDEX_ACCEL(i,j,k)] =
	      buffer2[INDEX_FFT(i+Offset[0], j+Offset[1], k+Offset[2])];
	  }
	}
      }
    ...
    } // end: if dim < GridRank (compute accel or potential)

  } // end: loop over acceleration dims
