...
#ifdef DC_OPT_SIBSUB
  static HierarchyEntry *TempHElist[MAX_NUMBER_OF_SUBGRIDS];
#endif
...
#ifdef DC_OPT_SIBSUB
		TempHElist[list->NumberOfSiblings] = current_link->HE_Data;
#endif
...
#ifdef DC_OPT_SIBSUB
    list->HE_List = new HierarchyEntry *[list->NumberOfSiblings];
#endif

    for (i = 0; i < list->NumberOfSiblings; i++) {
      list->GridList[i] = TempList[i];
#ifdef DC_OPT_SIBSUB
      list->HE_List[i] = TempHElist[i];
#endif
    }
  }
