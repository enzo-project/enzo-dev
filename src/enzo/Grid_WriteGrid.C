...
int grid::WriteGrid(FILE *fptr, char *base_name, int grid_id)
{
  ...
  if (NumberOfBaryonFields > 0) {
	...
	for (j = GridStartIndex[1]; j <= GridEndIndex[1]; j++)
      ...
      dset_id = H5Dcreate(file_id, "MgSiO3_Temperature", file_type_id, file_dsp_id, H5P_DEFAULT);
        if (io_log) fprintf(log_fptr, "H5Dcreate id: %"ISYM"\n", dset_id);
        if( dset_id == h5_error ){my_exit(EXIT_FAILURE);}
	...
	for (j = GridStartIndex[1]; j <= GridEndIndex[1]; j++)
      ...
      dset_id = H5Dcreate(file_id, "AC_Temperature", file_type_id, file_dsp_id, H5P_DEFAULT);
        if (io_log) fprintf(log_fptr, "H5Dcreate id: %"ISYM"\n", dset_id);
        if( dset_id == h5_error ){my_exit(EXIT_FAILURE);}
