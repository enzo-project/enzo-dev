...
int grid::FlagCellsToBeRefinedBySqrtSecondDeriv()
{
  ...
  if ( (MyProcessorNumber != ProcessorNumber) || (QuantumPressure != 1))
    return SUCCESS;
  ...
  /* find field number for wave function */
  ...
  for (int field = 0; field < NumberOfFields; field++) {
    ...
    if ((workfield == RePsiNum) || (workfield == ImPsiNum)) doField = true;
    //printf("workfield %d %d %d \n", workfield, RePsiNum, ImPsiNum);
    ...
    for (i = 0; i < size; i++){
      ...
      FlaggingField[i] += ((TopBuffer[i] > MinimumSecondDerivativeForRefinementThis) && (BaryonField[0][index]>20)) ? 1 : 0;
    }
  }  // end loop over field
