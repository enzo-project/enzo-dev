...
#ifdef DONOTCOMPILE
/* 
 * Remove mass from Grid after AP formation. 
 * The mass should be removed in a resolution dependent way. 
 */

int grid::RemoveMassFromGridAfterFormation(FLOAT* starpos,
					   int ParticleClass,
  ...
  
{
  ...
  ***********************************************************************/
  ...
  while (SphereTooSmall) { 
    ...
    this->GetEnclosedMassInShell(starpos, Radius-CellWidth[0][0], Radius, 
    ...
    if(POPII == ParticleClass) {
    
      ;

      
    }
    SphereTooSmall = false;
  }
