...
/* function prototypes */
extern "C" void PFORTRAN_NAME(cic_deposit_reject)(FLOAT *posx, FLOAT *posy,
                                                  FLOAT *posz, int *ndim, int *npositions,
                                                  float *densfield, float *field,
                                                  FLOAT *leftedge,
                                   ...
                                   int DepositField)
{
  ...
  FLOAT LeftEdge[MAX_DIMENSION], RightEdge[MAX_DIMENSION], OriginalLeftEdge[MAX_DIMENSION];
  ...
  if (ProcessorNumber != TargetGrid->ProcessorNumber)
  {
    ...
    for (dim = 0; dim < GridRank; dim++)
    {
      ...
      LeftEdge[dim] = max(LeftEdge[dim], OriginalLeftEdge[dim]);
      RightEdge[dim] = min(GridRightEdge[dim], OriginalLeftEdge[dim] * Dimension[dim] * CellSize);
      ...
      Dimension[dim] = int((RightEdge[dim] - LeftEdge[dim]) / CellSize) + 3;
      if (Dimension[dim] < 3)
        return SUCCESS;
    ...
    */
    ...
    if (SmoothField == FALSE)
    {
      ...
      else if (GravitySolverType == GRAVITY_SOLVER_APM)
      {
        ...
        /* With APM solver, use TSC deposition. */
        int EffectiveStartIndex[] = {0, 0, 0};

        if (GridRank == 1)
          DepositPositionsPileUpTSC1D(ParticlePosition, ParticleMassPointer,
                                      NumberOfParticles, DepositFieldPointer,
                                      LeftEdge, EffectiveStartIndex,
                                      Dimension, CellSize);

        if (GridRank == 2)
          DepositPositionsPileUpTSC2D(ParticlePosition, ParticleMassPointer,
                                      NumberOfParticles, DepositFieldPointer,
                                      LeftEdge, EffectiveStartIndex,
                                      Dimension, Dimension, CellSize);

        if (GridRank == 3)
          DepositPositionsPileUpTSC3D(ParticlePosition, ParticleMassPointer,
                                      NumberOfParticles, DepositFieldPointer,
                                      LeftEdge, EffectiveStartIndex,
                                      Dimension, Dimension, CellSize);

      } // end: if (GravitySolver == GRAVITY_SOLVER_FAST)
    }
    else
    { // SmoothField == TRUE
