...
#define FieldTypeIsDensity(A) (((A) == MetalSNIaDensity || (A) = MetalSNIIDensity) || ((A) >= TotalEnergy && (A) <= Velocity3) || ((A) >= kphHI && (A) <= kdissH2I) || ((A) >= RadiationFreq0 && (A) <= RaySegments) || ((A) >= Bfield1 && (A) <= AccelerationField3)) ? FALSE : TRUE)
