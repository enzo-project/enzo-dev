...
int grid::SolvePPM_DE_vg(float *gamma, int CycleNumber, int NumberOfSubgrids, 
              ...
              float MinimumSupportEnergyCoefficient)
{
  ...
  for (n = ixyz; n < ixyz+GridRank; n++) {
	...
	for (k = 0; k < GridDimension[2]; k++) {
	  if (this->xEulerSweep_vg(k, NumberOfSubgrids, SubgridFluxes, 
				GridGlobalStart, CellWidthTemp, GravityOn, 
				NumberOfColours, colnum, Pressure, gamma) == FAIL) {
	...
	for (i = 0; i < GridDimension[0]; i++) {
	  if (this->yEulerSweep_vg(i, NumberOfSubgrids, SubgridFluxes, 
				GridGlobalStart, CellWidthTemp, GravityOn, 
				NumberOfColours, colnum, Pressure, gamma) == FAIL) {
	...
	for (j = 0; j < GridDimension[1]; j++) {
	  if (this->zEulerSweep_vg(j, NumberOfSubgrids, SubgridFluxes, 
				GridGlobalStart, CellWidthTemp, GravityOn, 
				NumberOfColours, colnum, Pressure, gamma) == FAIL) {
