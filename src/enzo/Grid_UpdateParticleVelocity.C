...
int grid::UpdateParticleVelocity(float TimeStep)
{
  ...
  /* Some diagnostics */
  float time_now = ReturnTime();

  // TestSelfForce: there is only one particle
  if (ProblemType == 47) {
    if (NumberOfParticles > 1)
      ENZO_VFAIL("Error in grid->UpdateParticleVelocity, TestSelfForce: NumberofParticles is %"PISYM" > 1\n", NumberOfParticles);

    if (NumberOfParticles == 1) {
      printf("PARTICLE %"ESYM" %"ESYM" %"ESYM" %"ESYM" %"ESYM" %"ESYM" %"ESYM" %"ESYM" %"ESYM" %"ESYM"\n",
             time_now,
             ...
             ParticleAcceleration[0][0], ParticleAcceleration[1][0], ParticleAcceleration[2][0]);
      fflush(stdout);
    }
  }
