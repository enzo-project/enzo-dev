	       ...
	       float minsupecoef, float lapcoef)
{
  ...
  for (k = 0; k < kn; k++) {
    for (j = 0; j < jn; j++) {
      for (i = 0; i < in; i++){
          ...
          /*if (i==0){
          	p[IDX(i,j,k)] = (logd[IDX(0,j,k)]-2.0*logd[IDX(1,j,k)]+logd[IDX(2,j,k)])/pow(dx[i],2)/2.;
          	p[IDX(i,j,k)] = p[IDX(i,j,k)] + pow((-logd[IDX(0,j,k)]*3./2.+2.*logd[IDX(1,j,k)]-logd[IDX(2,j,k)]/2.)/dx[i],2)/4.;
          }
          if (i== in-1){
          	p[IDX(i,j,k)] = (logd[IDX(in-1,j,k)]-2.0*logd[IDX(in-2,j,k)]+logd[IDX(in-3,j,k)])/pow(dx[i],2)/2.;
          	p[IDX(i,j,k)] = p[IDX(i,j,k)] + pow((-logd[IDX(in-1,j,k)]*3./2.+2.*logd[IDX(in-2,j,k)]-logd[IDX(in-3,j,k)]/2.)/dx[i],2)/4.;
          }*/
          ...
          //p[IDX(i,j,k)] = p[IDX(i,j,k)]*pow(d[IDX(i,j,k)],0.001);
 ...
 /* for (k = ksm1; k <= kep1; k++) {
    for (j = jsm1; j <= jep1; j++) {
      for (i = is-2; i <= ie+1; i++){
	if (d[IDX(i,j,k)] <= 0.0) {
	  fprintf(stderr, "ZeusFDM error: u,v,w=%"GSYM",%"GSYM",%"GSYM"   i,j,k=%"ISYM",%"ISYM",%"ISYM"\n",
                  u[IDX(i,j,k)],v[IDX(i,j,k)],w[IDX(i,j,k)],i,j,k);
