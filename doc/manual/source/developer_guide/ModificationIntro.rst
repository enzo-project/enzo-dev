...
If you are experimenting with a code change or just debugging, then the patch directory, found in the top level of your Enzo directory, may be of use. Files put in here are compiled in preference to those in /src/enzo, so you can implement changes without overwriting the original code. For example, suppose you wish to check the first few values of the acceleration field as Enzo moves through EvolveLevel.C. Copy EvolveLevel.C from /src/enzo into /patch and put the appropriate printf statements throughout that copy of the code, then recompile. When you no longer want those changes, simply delete EvolveLevel.C from /patch and the next compile of the code will revert to using the original /src/enzo/EvolveLevel.C program. If you make adjustements to wish to keep, just copy the patch version of the code into /src/enzo to replace the original.
